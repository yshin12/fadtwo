# File name: sim-table3.R
# 
# A code for simulation studies in "Factor-driven Two-regime Regression"
# : This code generates Table 3 in the paper and save it as "../results/table3.txt"
#
...
table3 = data.frame(matrix(NA, 14,6))
colnames(table3) = c('Statistic', 'Method', 'T=200', 'T=300', 'T=400', 'T=500')
table3[,1] = c(rep('Min',3), rep('Median',3), rep('Mean',3), rep('Max',3), rep('Conv. Rate',2))
table3[,2] = c(rep(c('Iter(zeta=1.0)', 'Iter(zeta=0.1)', 'Joint'),4), 'Iter(zeta=1.0)', 'Iter(zeta=0.1)')
...
for (i.T in c(200,300,400,500)){
  ...
  # save the summary statistic for iter.(zeta=1.0) for the sample size T=i.T
  table3[c(1,4,7,10),paste('T=',i.T,sep='')] = round(summary(result[,2])[c(1,3,4,6)],2)
  # save the summary statistic for iter.(zeta=0.1) for the sample size T=i.T
  table3[c(2,5,8,11),paste('T=',i.T,sep='')] = round(summary(result[,1])[c(1,3,4,6)],2)
  # save the summary statistic for joint for the sample size T=i.T
  table3[c(3,6,9,12),paste('T=',i.T,sep='')] = round(summary(result[,3])[c(1,3,4,6)],2)
  # save the convergence rate for iter.(zeta=1.0)  
  table3[13,paste('T=',i.T,sep='')] = round(mean(abs(result[,5]-result[,6])<=eta),2)
...
save.image(file=paste("../results/o-sim-table3.RData",sep=''))
